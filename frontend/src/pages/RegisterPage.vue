...
async function handleSubmit() {
  ...
  try {
    ...
    if (typeof error === 'object' && error !== null && 'status' in error) {
      ...
      if (Reflect.get(error, 'status') === 422 && typeof data === 'object' && data !== null && 'errors' in data) {
...
<template>
  <main class="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-12 dark:bg-slate-950">
    <div class="w-full max-w-2xl rounded-[32px] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-black/20">
      ...
      <form class="mt-8 grid gap-5 md:grid-cols-2" @submit.prevent="handleSubmit">
        <div>
          ...
          <p v-if="fieldErrors.name" class="mt-2 text-sm text-red-700 dark:text-red-300">{{ fieldErrors.name[0] }}</p>
        </div>
        <div>
          <label for="business_name" class="text-label dark:text-slate-200">Établissement</label>
          <input id="business_name" v-model="form.business_name" type="text" class="mt-2 input-field" />
          <p v-if="fieldErrors.business_name" class="mt-2 text-sm text-red-700 dark:text-red-300">{{ fieldErrors.business_name[0] }}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <input id="phone" v-model="form.phone" type="tel" placeholder="+33612345678" class="mt-2 input-field" />
        ...
        <div>
          <label for="password_confirmation" class="text-label dark:text-slate-200">Confirmation</label>
          <input id="password_confirmation" v-model="form.password_confirmation" type="password" class="mt-2 input-field" />
        </div>

        <p v-if="generalError" class="md:col-span-2 rounded-2xl bg-red-100 px-4 py-3 text-sm text-red-800 dark:bg-red-900/20 dark:text-red-300">
          {{ generalError }}
        </p>

        <div class="md:col-span-2 flex flex-wrap items-center justify-between gap-3 pt-2">
          <RouterLink to="/login" class="text-body-sm font-semibold text-emerald-700 dark:text-emerald-400">
